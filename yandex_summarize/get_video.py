...
class YaDiskAPI:
    ...
    def __init__(self):
        ...
        logging.info('Session creation successful')
    ...
    def get_video(self, url: str) -> None:
        ...
        try:
            sub = self.session.get(self.get_download_url, headers=self.headers)
            self.download_url = json.loads(sub.text)["href"]

            self.raw_video_data = self.session.get(self.download_url, headers=self.headers)
